...
@Author: 27
@LastEditors: 27
@Date: 2020-03-03 08:36:35
@LastEditTime: 2020-03-03 08:46:18
@FilePath: /Algorithms_Note/algorithms_practice/验证IP.py
...
所以， 2001:db8:85a3:0:0:8A2E:0370:7334 也是一个
有效的 IPv6 address地址 (即，忽略 0 开头，忽略大小写)。
...
class Solution(object):
    def validIPAddress(self, IP):
        ...
        :rtype: str
        """
        return self.ip4_or_ip6_or_Neither(IP)
    
    def ip4_or_ip6_or_Neither(self, IP):
        if '.' in IP:
            return self.is_ip4(IP)
        elif ':' in IP:
            return self.is_ip6(IP)
        else:
            return "Neither"
    def is_ip4(self, IP):
        for i in IP.split('.'):
